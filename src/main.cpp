...
void setup()
{
    ...
    if (LittleFS.begin())
    {
        ...
        if (LittleFS.exists(F("/config.json")))
        {
            ...
            if (configFile)
            {
                ...
                {
                    Serial.println(F("\nparsed json"));
                    strcpy(ntp_server1, jDoc["ntp_server1"]);
                    strcpy(ntp_server2, jDoc["ntp_server2"]);
                    strcpy(ntp_server3, jDoc["ntp_server3"]);
                }
            }
            else
            {
    ...
    if (shouldSaveConfig)
    {
        ...
        StaticJsonDocument<256> jDoc;
        jDoc["ntp_server1"] = ntp_server1;
        jDoc["ntp_server2"] = ntp_server2;
        jDoc["ntp_server3"] = ntp_server3;
